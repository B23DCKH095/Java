...
class SV implements Comparable<SV>{
    ...
    public int compareTo(SV other){
        int ss = Double.compare(Double.parseDouble(other.tongdiem), Double.parseDouble(this.tongdiem));
...
public class J05059 {
    public static void main(String[] args) {
        ...
        String diemc = new String();
        for (SV tmp : ds) {
            if(q > 0){
                tmp.trangthai = "TRUNG TUYEN";
                diemc= tmp.tongdiem;
            } else tmp.trangthai = "TRUOT";
            q--;
        }
        System.out.println(String.format("%.1f",Double.parseDouble(diemc)));
        for(SV tmp : ds){
            if(Double.parseDouble(tmp.tongdiem )>= Double.parseDouble(diemc)) tmp.trangthai= "TRUNG TUYEN";
        }
        for(SV tmp : ds) System.out.println(tmp);
    }
}
